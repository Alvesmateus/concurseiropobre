(function() {
    ...
    if (!document.getElementById(cssId)) {
        ...
        style.textContent = `
            ...
            .g-filter-widget-instance { font-family: 'Google Sans', Roboto, Arial, sans-serif; background: #ffffff; border: 1px solid #c4c7c5; width: 100%; position: relative; z-index: 1000; box-shadow: 0 1px 2px rgba(0,0,0,0.05); border-radius: 8px; overflow: hidden; margin-top:-10px; margin-bottom: 10px; }
    ...
    class FilterQuestoesWidget {
        constructor(container, options = {}) {
            ...
            this.options = Object.assign({
                fullWidth: false,
                jsonUrl: 'https://raw.githubusercontent.com/Alvesmateus/concurseiropobre/refs/heads/main/filtrodequestoes/filtrodequestoes.json', // URL do JSON no GitHub
                config: null, // Será carregado do JSON
                tagFixa: "questão"
            }, options);
        ...
        async init() {
            ...
            try {
                ...
                // Inicializar widget com configuração carregada
                this.renderHTML();
                this.bindEvents();
                ...
                // Usar configuração fallback em caso de erro
                this.useFallbackConfig();
            }
        }

        async loadConfig() {
            // Se já temos config, não precisa carregar
            ...
            try {
                ...
                if (!config.materias || !config.banca || !config.assuntos) {
                    throw new Error('Estrutura do JSON inválida');
                }
                
                this.options.config = config;
            } catch (error) {
                console.warn('Falha ao carregar JSON do GitHub. Usando fallback:', error);
                throw error; // Repassar erro para ser tratado no init
            }
        }

        useFallbackConfig() {
            // Configuração fallback caso o JSON não carregue
            this.options.config = {
            ...
            // Continuar inicialização com fallback
            this.renderHTML();
            this.bindEvents();
            this.renderBubbles('materias', this.options.config.materias, 'm');
            this.renderBubbles('banca', this.options.config.banca, 'b');
        ...
        showLoading() {
            this.container.innerHTML = `
                <div class="g-filter-widget-instance${this.options.fullWidth ? ' full-width' : ''}">
                    <div class="g-header">
                    ...
                    <div class="g-panel">
                        <div class="g-loading">Carregando filtros...</div>
                    </div>
                </div>
            `;
        }
    ...
    async function initWidgets() {
        ...
        for (const container of containers) {
            ...
            if (!container.dataset.widgetInitialized) {
                const widget = new FilterQuestoesWidget(container);
